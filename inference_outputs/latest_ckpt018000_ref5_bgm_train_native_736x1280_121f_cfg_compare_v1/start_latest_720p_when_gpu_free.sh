...
ROOT="inference_outputs/latest_ckpt018000_ref5_bgm_train_native_736x1280_121f_cfg_compare_v1"
SOURCE_INPUT_ROOT="inference_outputs/ckpt010500_ref5_bgm_train_720p_cfg_compare_v1/inputs"
CKPT="logs/wan2.2_5B_fp8_audio_20260624_172511/checkpoint_model_018000/model.pt"
WIDTH=736
HEIGHT=1280
FRAME_NUM=121
FPS=25
...
cat > "$ROOT/run_config.json" <<EOF
{
  ...
  "note": "Native 720p-style inference at 736x1280; no post-resize."
}
EOF

echo "[$(date -Is)] Waiting for GPUs 0 and 1 to become available for latest ckpt native ${WIDTH}x${HEIGHT}, ${FRAME_NUM} frames, ${FPS}fps..." | tee -a "$ROOT/logs/launcher.log"
while true; do
...
echo "[$(date -Is)] GPUs are available. Launching latest ckpt native ${WIDTH}x${HEIGHT}, ${FRAME_NUM}f inference." | tee -a "$ROOT/logs/launcher.log"
