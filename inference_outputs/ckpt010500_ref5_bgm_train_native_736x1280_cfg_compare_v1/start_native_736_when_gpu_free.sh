...
ROOT="inference_outputs/ckpt010500_ref5_bgm_train_native_736x1280_cfg_compare_v1"
...
echo "[$(date -Is)] Waiting for GPUs 0 and 1 to become available..." | tee -a "$ROOT/logs/launcher.log"
while true; do
...
echo "[$(date -Is)] GPUs are available. Launching native ${WIDTH}x${HEIGHT} inference." | tee -a "$ROOT/logs/launcher.log"
...
CUDA_VISIBLE_DEVICES=0 PYTHONUNBUFFERED=1 python scripts/infer_audio_i2v.py \
  ...
  --output_dir "$ROOT/no_cfg_native_736x1280" \
  --width "$WIDTH" \
  --height "$HEIGHT" \
  --frame_num "$FRAME_NUM" \
  --fps "$FPS" \
  ...
  --pairing match \
  --max_jobs "$MAX_JOBS" \
  > "$ROOT/logs/no_cfg_native_736_stdout.log" \
  2> "$ROOT/logs/no_cfg_native_736_stderr.log" &
PID0=$!
...
CUDA_VISIBLE_DEVICES=1 PYTHONUNBUFFERED=1 python scripts/infer_audio_i2v.py \
  ...
  --ref_dir "$ROOT/inputs/ref_5" \
  --audio_dir "$ROOT/inputs/audio_5" \
  --output_dir "$ROOT/textcfg3_native_736x1280" \
  --width "$WIDTH" \
  --height "$HEIGHT" \
  --frame_num "$FRAME_NUM" \
  --fps "$FPS" \
  ...
  --pairing match \
  --max_jobs "$MAX_JOBS" \
  > "$ROOT/logs/textcfg3_native_736_stdout.log" \
  2> "$ROOT/logs/textcfg3_native_736_stderr.log" &
PID1=$!

echo "$PID0" > "$ROOT/logs/no_cfg_native_736.pid"
echo "$PID1" > "$ROOT/logs/textcfg3_native_736.pid"
